...
int main() {
    ...
    while (getline(cin, line) && line != "") {
        string s;
        for (char c : line) if (c != ' ') s.push_back(c);
        ll n = sz(s);
        SuffixArray A(s);
        UF uf(n+1);
        vi order(n+1);
        rep(i, 0, n+1) order[i] = i;
        sort(all(order), [A](const ll a, const ll b) {
            return A.lcp[a] > A.lcp[b];
        });
        vi res(n+1, 0);
        for (ll i : order) {
            if (i > 0 && A.lcp[i-1] >= A.lcp[i]) uf.join(i, i-1);
            if (i < n && A.lcp[i+1] >= A.lcp[i]) uf.join(i, i+1);
            res[A.lcp[i]] = max(res[A.lcp[i]], uf.size(i));
        }
        rep(i, 1, n+1) 
            if (res[i]) printf("%lld\n", res[i]+1);
            else break;
